...
const MaterialSchema = require("../models/material.model.js");
...
module.exports = {
  CreateVoucher: async (req, res) => {
    try {
      const { titulo, stores, duracion, puntosVale, recycledMaterials, usuario } =
        req.body;
    ...
    // Restar los puntosVale al usuario correspondiente
    const user = await BaseUser.findById(usuario);
    if (!user) {
      return res.status(404).json({ error: "Usuario no encontrado" });
    }
    user.puntos -= puntosVale; // Restar los puntosVale del voucher al usuario
    await user.save();
